...
class ArticleList extends React.Component {
  ...
  async fetchArticle({ reset = false }) {
    let searchCond = "";
    if (this.state.form.search !== "")
      searchCond = `&keyword=${this.state.form.search}`;
    let response = await axios.get(
      `${Config.API_PUBLIC_URL}/article?is_popular=true&current_page=${
        reset ? 1 : this.state.articleMeta.page + 1
      }&limit=8${searchCond}`
    );
    try {
      this.setState({
        article: reset
          ...
          : [...this.state.article, ...response.data.data.article],
  ...
  handleFormChange(event) {
    ...
    if (name === "search") {
      if (this.state.fetchArticleCallback !== null)
        clearTimeout(this.state.fetchArticleCallback);
      this.setState({
        fetchArticleCallback: setTimeout(() => {
          this.setState({ isLoading: true });
          this.fetchArticle({ reset: true }).then(() => {
  ...
  viewNewestEbook() {
    let result = [];
    let haveMore = false;
    for (let i = 0; i < this.state.ebookList.length; i++) {
      if (result.length >= this.state.maxNewestEbook) {
        haveMore = true;
        break;
      }
      let item = this.state.ebookList[i];
      let tagExist = false;
      ...
      if (
        ...
        result.push(this.state.ebookList[i]);
      }
    }
            ...
            <div className="container">
              ...
              {this.state.topicList.length > 0 && (
                <div
                  ...
                  style={{
                    display: this.state.form.search === "" ? "inherit" : "none",
                  }}
                >
                  <div style={{ minWidth: "32px", minHeight: "32px" }}></div>
                ...
                <div className="newest-article">
                  ...
                  {this.state.articleMeta.page <
                    ...
                    <div className="text-center">
                      <button className="add-more" onClick={this.fetchArticle}>
                        Selanjutnya
                      </button>
                    </div>
                  )}
              ...
              {this.state.form.search !== "" && this.state.type === "EBOOK" && (
                <div className="newest-article">
                  <div style={{ minWidth: "32px", minHeight: "32px" }}></div>
                  {this.state.form.search === "" && (
                    <div className="title">Ebook Terbaru</div>
                  )}
                  {this.state.form.search !== "" && (
                    <div className="title">
                      Hasil Pencarian untuk &quot;{this.state.form.search}&quot;
